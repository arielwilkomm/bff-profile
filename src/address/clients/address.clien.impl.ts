...
import { AxiosInstance, AxiosRequestConfig, AxiosResponse, AxiosHeaders } from 'axios';
...
export class AddressClientImpl implements IAddressClient {
  ...
  async getAddress(
    ...
    Logger.info(`URL: ${url}`);
    return this.handleRequest<AddressRecordDTO>('getAddress', () =>
      ...
      return await firstValueFrom<AxiosResponse<T>>(
        requestFn().pipe(
          map((response) => {
            ...
            Logger.debug(`Response for ${method} - CPF: ${cpf} - Data: ${JSON.stringify(axiosResponse.data)}`);
